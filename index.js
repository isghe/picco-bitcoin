		...
		const ClassController = function (theNow) {
			...
			self.matrixToTable = (header, matrix, {threshold, absoluteThreshold, satoshiColIndex, separatorLabels}) => {
				...
				const insertSeparator = label => {
					const domSep = self.util.createElement(['tr', null, ['separator']]);
					domSep.append(self.util.createElement(['td', {textContent: label, colSpan: header.length}]));
					domTable.append(domSep);
				};
				...
				matrix.forEach(row => {
					...
					if (satoshi > absoluteThreshold) {
						domRow.classList.add('lost-absolute');
					}
